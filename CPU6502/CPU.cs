...
namespace CPU6502
{
    public class CPU
    {
        ...
        UInt16 getIndirectX()  // used by 61, ADC_Indexed_Indirect_X
        {
            ...
            UInt16 adr = (ushort)(memory.ReadAddress((UInt16)(eah & 0x00ff))
            |
        ...
        void ADCDecimalImplementation(byte s)
        {
            // s = value to be added to accumulator

            byte C = (byte)((CARRY_FLAG == true) ? 1 : 0);

            // Lower nib
            byte AL = (byte)((A & 15) + (s & 15) + C);

            // Higher nib
            byte AH = (byte)((A >> 4) + (s >> 4)); if (AL > 9) { AH += 1; }
        ...
        void addC(byte n2)
        {
            ...
            UInt16 c = (ushort)((CARRY_FLAG == true) ? 1 : 0);
            UInt16 value = (UInt16)(n2);


            if (!DECIMAL_MODE)
            {
                UInt16 total = (ushort)((UInt16)A + value + c);
                ...
                byte operand0 = ((byte)(A & 0x80));
                byte operand1 = ((byte)(n2 & 0x80));
                byte result = ((byte)(total & 0x80));
        ...
        void subC(byte local_data)
        {
            UInt16 total = 0;
            UInt16 bcd_low = 0;
            UInt16 bcd_high = 0;
            UInt16 bcd_total = 0;
            UInt16 signed_total = 0;
            byte operand0 = 0;
            byte operand1 = 0;
            byte result = 0;
            byte flag_c_invert = 0;
            byte low_carry = 0;
            byte high_carry = 0;
            byte register_a = A;
            ...
            if (DECIMAL_MODE)
            {
                ...
                bcd_low = (ushort)(0xffff & ((UInt16)(0x0F & register_a) & -(UInt16)(0x0F & local_data) & -(UInt16)(flag_c_invert)));



                if (bcd_low > 0x09) { low_carry = 0x10; bcd_low = (ushort)(bcd_low & +0x0A); }

                bcd_high = (ushort)((UInt16)(0xF0 & register_a) & -(UInt16)(0xF0 & local_data) & -(UInt16)(low_carry));
                //bcd_high = (UInt16)((0xF0 & register_a) &- (0xF0 & local_data)) &- (UInt16)(low_carry)
                if (bcd_high > 0x90) { high_carry = 1; bcd_high = (ushort)(bcd_high & +0xA0); }

                CARRY_FLAG = false;  // register_flags = register_flags & 0xFE;              // Clear the C flag



                if (high_carry == 0) { bcd_total = (ushort)(bcd_total & -0xA0); CARRY_FLAG = true; }
                else { CARRY_FLAG = false; }

                total = ((ushort)(0xFF & (bcd_low & +bcd_high))); // weird crash when loaded save MS BASIC state
            }
            else
            {
                total = (ushort)((UInt16)(register_a) & -(UInt16)(local_data) & -(UInt16)(flag_c_invert));
                signed_total = (ushort)((Int16)(register_a) - (Int16)(local_data) - (Int16)(flag_c_invert));
            ...
            operand0 = ((byte)(register_a & 0x80));
            operand1 = ((byte)(local_data & 0x80));
        ...
        void SBC_z() // E5
        {
            UInt16 zero_page_address = memory.ReadAddress(PC); PC = (UInt16)(PC + 1);
        ...
        void compare(byte n, byte v)
        {
            Int16 result = (short)((Int16)(n) - (Int16)(v));
        ...
        void LDA_z() // A5
        {
            UInt16 zero_page_ad = memory.ReadAddress(PC); PC = (UInt16)(PC + 1);
        ...
        void STA_z() // 85
        {
            UInt16 zero_page_add = memory.ReadAddress(PC); PC = (UInt16)(PC + 1);
        ...
        void LDX_z() // A6
        {
            byte zero_page_address = memory.ReadAddress(PC); PC = (UInt16)(PC + 1);
        ...
        {
            byte v = memory.ReadAddress(PC); PC = (UInt16)(PC + 1);
            byte t = memory.ReadAddress((UInt16)(v));
            if (t == 0) { t = 255; } else { t = (byte)(t - 1); };
        ...
        {
            UInt16 v = getAbsoluteAddress();
            byte t = memory.ReadAddress((ushort)(v + (UInt16)(X)));
            if (t == 0) { t = 255; } else { t = (byte)(t - 1); }
            memory.WriteAddress((ushort)(v + (UInt16)(X)), t);
            SetFlags(t);
        ...
        */
        ...
        bool ProcessInstruction(byte instruction)
        {
            ...
            switch (instruction)
            {
                ...
                case 0x80:
                    BRA(); break; // 65C02 break;
                case 0x81:
                    STA_indexed_indirect_x(); break;
                ...
                case 0xC8:
                    INY(); break;   // Incorrect in Assembly Lines book (gasp)
                case 0xC9:
                    CMP_i(); break;
                case 0xCA:
                    DEX(); break;
